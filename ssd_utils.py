...
class PriorUtil(object):
    ...
    def __init__(
        ...
        if isQuads is None:
            if hasattr(model, "isQuads"):
                isQuads = model.isQuads
            else:
                isQuads = True
        self.isQuads = isQuads

        if isRbb is None:
            if hasattr(model, "isRbb"):
                isRbb = model.isRbb
            else:
                isRbb = True
        self.isRbb = isRbb
    ...
    def encode(self, gt_data, overlap_threshold=0.45, debug=False):
        ...
        gt_boxes = self.gt_boxes = np.copy(
            gt_data[:, :4]
    ...
    def plot_assignment(self, map_idx):
        ...
        # ground truth
        boxes = self.gt_boxes
        boxes_x = (boxes[:, 0] + boxes[:, 2]) / 2.0 * image_w
        boxes_y = (boxes[:, 1] + boxes[:, 3]) / 2.0 * image_h
        for box in boxes:
        ...
        for idx, box_idx in self.match_indices.items():
            if idx >= self.map_offsets[map_idx] and idx < self.map_offsets[map_idx + 1]:
                ...
                plt.plot(
                    [x, boxes_x[box_idx]], [y, boxes_y[box_idx]], "-r", linewidth=1
                )
                ax.add_patch(
                    plt.Rectangle(
                        (x - w / 2, y - h / 2),
    ...
    def plot_results(
        ...
        if gt_data is not None:
            for box in gt_data:
                ...
                xy_rec = to_rec(box[:4], image_size)
                ax.add_patch(
                    plt.Polygon(xy_rec, fill=True, color=color, linewidth=1, alpha=0.3)
                )
