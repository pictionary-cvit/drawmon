...
class TBPPFocalLoss(object):
    ...
    def __init__(self, lambda_conf=1000.0, lambda_offsets=1.0, isQuads=False, isRbb=True, aabb_weight = 1., rbb_weight = 1., quad_weight = 1., decay_factor = 10., priors_xy=None, priors_wh=None, priors_variances=None, img_wd=512.0, img_ht=512.0, aabb_diou=True, rbb_diou=True, isfl=True, neg_pos_ratio=3.0):
        ...
        self.metric_names = [ 'loss',
                'conf_loss', 'loc_loss', 'loc_loss_aabb',
                'precision', 'recall', 'fmeasure', 'accuracy',
                'num_pos', 'num_neg', 'aabb_weight', 'rbb_weight'
            ]
        if self.isQuads:
    ...
    def rbox3_to_polygon(self, rbox):
        ...
        dx = -h*tf.math.cos(alpha) / 2.
        dy = -h*tf.math.sin(alpha) / 2.
        xy = tf.reshape(tf.concat([x1-dx,y1-dy,x2-dx,y2-dy,x2+dx,y2+dy,x1+dx,y1+dy], axis=0), (-1,2))
    ...
    def rbox3_to_rbox(self, rbox):
        ...
        w = (tf.norm(dt) + tf.norm(db)) / 2.
        # height is distance from center to top edge plus distance form center to bottom edge
        tlsubc = tl-c
        brsubc = br-c
        h = tf.norm(dt[0]*tlsubc[1] - dt[1]*tlsubc[0])/(tf.norm(dt)+eps) + tf.norm(db[0]*brsubc[1]- db[1]*brsubc[0])/(tf.norm(db)+eps)
        ...
        theta = (tf.math.atan2(dt[0], dt[1]) + tf.math.atan2(db[0], db[1])) / 2.
    ...
    def to_box(self, offsets):
        ...
        offsets = tf.concat([boxes_xy - boxes_wh / 2., boxes_xy + boxes_wh / 2.], axis=1) # xmin, ymin, xmax, ymax
    ...
    def to_rbox(self, offsets):
        ...
        rboxs_x1y1 = self.priors_xy + offsets[:,0:2] * self.priors_wh * self.variances_xy
        rboxs_x2y2 = self.priors_xy + offsets[:,2:4] * self.priors_wh * self.variances_xy
        rboxs_h = tf.expand_dims(tf.math.exp(offsets[:,4] * self.variances_wh[:,1]) * self.priors_wh[:,1], axis=-1)
    ...
    def riou(self, boxes):
        ...
        return 1 - tf.squeeze(tf.squeeze(iou_rotate_calculate(tf.expand_dims(box1, axis=0), tf.expand_dims(box2, axis=0)), axis=0), axis=0)
    ...
    def compute(self, y_true, y_pred, img_wd=512.0, img_ht=512.0):
        ...
        if aabb_diou:
            ...
            loc_loss_aabb = ciou_loss(y_true_aabb*(img_wd, img_ht, img_wd, img_ht), y_pred_aabb*(img_wd, img_ht, img_wd, img_ht))
            
            pos_loc_loss_aabb = tf.reduce_sum(loc_loss_aabb * pos_mask_float) # only for positives
        ...
        if self.isQuads:
            ...
            pos_loc_loss_qbb = tf.reduce_sum(loc_loss_qbb * pos_mask_float) # only for positives
        ...
        if self.isRbb:
            ...
            # temp DIoU Error
            loss_rbb_diou_error = smooth_l1_loss(loc_true[:,4:9], loc_pred[:,4:9]) # /10 to keep range same
            ...
            if rbb_diou==True:
                ...
                y_true_rbb_poly = tf.map_fn(self.rbox3_to_polygon, y_true_rbb) # (N, 4, 2)
                ...
                loss_rbb_iou = tf.map_fn(self.riou, tf.concat([y_true_rbb, y_pred_rbb], axis=1))
        ...
        print('{'+' '.join(['"'+n+'": '+n+',' for n in self.metric_names])+'}')
