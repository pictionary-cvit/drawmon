...
for k in tqdm(range(initial_epoch, epochs), "total", leave=False):
    ...
    with val_summary_writer.as_default():
        for metric in val_loss:
            tf.summary.scalar(
                str(metric), val_loss[metric] / num_val_batches, step=iteration
            )

        first_renders = renderPreds(
            first_batch_images, first_batch_preds, first_batch_gt
        )
        best_renders = renderPreds(best_batch_images, best_batch_preds, best_batch_gt)
        worst_renders = renderPreds(
            worst_batch_images, worst_batch_preds, worst_batch_gt
        )
