...
def multibox_head(
    ...
    if isQuads and isRbb:
        predictions = concatenate(
            [mbox_loc, mbox_quad, mbox_rbox, mbox_conf], axis=2, name="predictions"
        )
    elif isRbb:
        predictions = concatenate(
            [mbox_loc, mbox_rbox, mbox_conf], axis=2, name="predictions"
        )
    elif isQuads:
        predictions = concatenate(
...
def TBPP512(
    ...
    model.shifts = [
        [(0.0, -0.25)] * len(aspect_ratios) + [(0.0, 0.25)] * len(aspect_ratios)
    ] * num_maps
...
def multibox_head_separable(
    ...
    for i in range(len(source_layers)):
        ...
        if isQuads:
            name3 = name + "_mbox_quad"
            x3 = SeparableConv2D(num_priors[i] * 8, (3, 5), padding="same", name=name3)(
                x
            )
            x3 = Flatten(name=name3 + "_flat")(x3)
            mbox_quad.append(x3)
        ...
        if isRbb:
            name4 = name + "_mbox_rbox"
            x4 = SeparableConv2D(num_priors[i] * 5, (3, 5), padding="same", name=name4)(
                x
            )
            x4 = Flatten(name=name4 + "_flat")(x4)
            mbox_rbox.append(x4)
...
def TBPP512_dense_separable(
    ...
    # custom body
    x = input_tensor = Input(shape=input_shape)
    source_layers = ssd512_dense_separable_body(
        x,
    ...
    num_priors = [2 * len(aspect_ratios)] * num_maps
    normalizations = [1] * num_maps
    output_tensor = multibox_head_separable(
        source_layers,
        num_priors,
        normalizations,
        softmax,
