...
parser.add_argument(
    "--split", type=str, choices=["val", "test"], required=False, default=data_split
)
...
gen_val = InputGenerator(
    ds_val, prior_util, batch_size, 2, encode=True, overlap_threshold=0.5
)
...
pycoco_metric = PycocoMetric(iou_threshold=0.5, confidence_threshold=0.3, top_k=200)
...
for fp in tqdm(range(len(weight_files))):
    ...
    with val_summary_writer.as_default():
        ...
        tf.summary.scalar(
            "F1",
            2 * (metrics[1][1]) * (metrics[7][1]) / (metrics[1][1] + metrics[7][1]),
            step=fp + 1,
        )
        # print()
