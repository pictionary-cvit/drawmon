...
class PriorUtil(SSDPriorUtil):
    ...
    def encode(
        self, gt_data, overlap_threshold=0.5, debug=False, quads=False, num_classes=2
    ):
        ...
        gt_one_hot[
            range(len(gt_one_hot)), gt_class_idx
        ] = 1  # one_hot classes including background
        ...
        max_idxs = np.argmax(
            gt_iou, axis=1
        )  # id of gt that has max-iou for each prior, shape: (num_priors)
        max_val = gt_iou[
            np.arange(num_priors), max_idxs
        ]  # max-iou for each prior, shape: (num_priors)
        prior_mask = max_val > overlap_threshold
        match_indices = max_idxs[
            prior_mask
        ]  # indices of gt that has been assigned to some prior
        ...
        ) / 2.0  # coordinates of center of aabb:gt_boxes
        gt_wh = gt_boxes[:, 2:4] - gt_boxes[:, 0:2]  # width and ht of aabb:gt_boxes
        gt_xy = gt_xy[
            match_indices
        ]  # get center of only aabb:gt's that has been assigned to some prior
        gt_wh = gt_wh[match_indices]
        gt_polygons = gt_polygons[
            match_indices
        ]  # get quad corresponding to aabb:gt's that has been assigned to some prior
        ...
        offsets = np.zeros(
            (num_priors, 4)
        )  # init offeset for each prior with [0, 0, 0, 0]
        ...
        )  # gives coords of priors as (xmin, ymin, xmax, ymax)
        # ref = np.tile(priors_xy, (1,4))
        ref = priors_xy_minmax[
            :, (0, 1, 2, 1, 2, 3, 0, 3)
        ]  # corner points => 1st corner(xmin, ymin) i.e (0,1) of priors_xy_minmax, 2nd corner (xmax, ymin) i.e (2, 1)
            ...
            if len(boxes_to_process) > 0:
                ...
                if self.isQuads and self.isRbb:
                    c_pred = np.concatenate(
                        (good_boxes, good_quads, good_rboxs, good_confs, labels), axis=1
                    )
                elif self.isRbb:
                    c_pred = np.concatenate(
                        (good_boxes, good_rboxs, good_confs, labels), axis=1
                    )
                elif self.isQuads:
                    c_pred = np.concatenate(
    ...
    def plot_results(
        ...
        if gt_data is not None:
            for box in gt_data:
                ...
                ax.add_patch(
                    plt.Polygon(xy, fill=True, color=color, linewidth=1, alpha=0.3)
                )
        ...
        if gt_data_decoded is not None:
            for r in gt_data_decoded:
                ...
                if self.isQuads:
                    ...
                    rbox = r[4:9]
                    plot_box(
                        rbox3_to_polygon(rbox) * (w, h), box_format="polygon", color="y"
                    )
                else:
        ...
        for r in results:
            ...
            if self.isQuads:
                plot_box(
                    np.reshape(quad, (-1, 2)) * (w, h), box_format="polygon", color="r"
                )

            if self.isRbb:
                plot_box(
                    rbox3_to_polygon(rbox) * (w, h), box_format="polygon", color="g"
                )
                plt.plot(
                    rbox[[0, 2]] * (w, w), rbox[[1, 3]] * (h, h), "oc", markersize=4
                )
